...
@app.route('/users/blog', methods=['POST'])
def add_blog():
    cur = mysql.connection.cursor()
    title = request.get_json()['title']
    descr = request.get_json()['descr']

    cur.execute("INSERT INTO blog_db.blog (title, descr) VALUES ('" +
                 str(title) + "', '" +
                str(descr) + "')")
    mysql.connection.commit()
    result = {'title': title,
...
@app.route('/user/all_blog', methods=['GET'])
def get_all_blog():
    cur = mysql.connection.cursor()
    cur.execute("SELECT * FROM blog_db.blog")
    rv = cur.fetchall()
    return jsonify(rv)
